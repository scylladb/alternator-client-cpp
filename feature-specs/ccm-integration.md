...
Alternator C++ integration tests.

The keywords **must**, **must not**, **should**, and **may** are normative.

## Purpose and scope

The harness supplies a real Scylla cluster without requiring Docker or another container runtime.
It supports a shared default cluster for ordinary integration tests and an exclusive cluster for
tests that change topology or process state. One test process owns at most one physical cluster at
a time.
...
`SCYLLA_CCM_ROOT` may select the harness-owned state root. Its default must be private, per-user,
and shared by the user's test processes. Runs and address reservations must be derived beneath that
root. Concurrent processes must use the same root to coordinate address selection; distinct
override roots intentionally do not share reservations. `SCYLLA_CCM_DIAGNOSTICS_DIR` selects an
external artifact directory that must not overlap live state.
...
The final node must not be removed. Node handles from another cluster, or stale handles for a
removed node, must be rejected. Added nodes must receive all configured YAML overrides and TLS
...
from an in-flight command survives process termination. Timed-out or interrupted commands must
have their process groups terminated before rollback begins.

Before destructive cluster removal, the harness must snapshot command output, configuration, and
node logs without copying private keys. If the snapshot cannot be transferred to the external
diagnostics directory, live state must be retained for a later attempt.

Normal shutdown must attempt immediate cleanup. At startup, the harness must inspect previous runs
under the configured root and clean runs whose recorded PID, process start time, and boot identity
are no longer active. It must terminate same-user processes bearing the exact run marker before
invoking bounded CCM removal. The run and its address reservation may be deleted only after cleanup
succeeds.
...
### Build and integration-suite ownership

The existing integration suite must obtain endpoints, topology labels, credentials, and CA
material from a reusable CCM lease instead of fixed Docker networking or externally provisioned
containers. Provisioning contract tests and ordinary client integration tests must run in the
foreground, and failure from either phase must fail the integration target.

Cluster lifecycle and crash recovery must live in the C++ test harness. No standalone shell script
may own provisioning, cleanup, or recovery.

## Interactions with other features

### TLS configuration

Generated CA and node certificates must support custom trust without disabling certificate or
hostname validation. Certificates must contain each node's loopback IP as a subject alternative
name. Added nodes must receive certificates before they start.
...
- Repeated or moved-from lease destruction must not release ownership twice.
...
cluster per process, replace idle incompatible state, and fail incompatible requests immediately
while leases are active. It must not depend on host-memory detection, wait queues, or LRU eviction.
